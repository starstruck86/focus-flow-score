ALTER TABLE public.ki_mastery ADD COLUMN IF NOT EXISTS next_review_at timestamptz;
...
CREATE OR REPLACE FUNCTION public.get_next_ki_for_dimension(p_user_id uuid, p_spider_dimension text, p_limit integer DEFAULT 1)
 RETURNS TABLE(id uuid, title text, tactic_summary text, why_it_matters text, when_to_use text, when_not_to_use text, example_usage text, framework text, chapter text, sub_chapter text, spider_dimension text, confidence_score integer, active boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  FROM (
    ...
    WHERE user_id = p_user_id
      AND spider_dimension = p_spider_dimension
