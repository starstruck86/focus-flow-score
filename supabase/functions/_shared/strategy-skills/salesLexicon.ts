/**
 * Sales Lexicon — Phase 3B (pure, server-authoritative).
 *
 * Deterministic vocabulary bridge between BUSINESS language (what users
 * type: "platform consolidation", "guest experience", "renewal") and
 * SALES METHODOLOGY language (what the library is indexed in: "discovery",
 * "POV", "business case", "value realization").
 *
 * No fuzzy matching, no embeddings, no model calls. Pure tables.
 *
 * Versioned (LEXICON_VERSION) so trace + planHash invalidate cleanly when
 * the table evolves.
 */
...
export interface LexiconEntry {
  /** Substring trigger (lowercased, token-bounded matched). */
  trigger: string;
  /** Sales-vocabulary expansions emitted when trigger matches. */
  expansions: ReadonlyArray<string>;
  /** For trace: human-readable rule id. */
  rule: string;
}

/**
 * Frozen, insertion-ordered. Iteration order = match order = trace order.
 *
 * KEEP THIS LIST DELIBERATE. Each entry must:
 *   - use a token (>=4 chars) unique enough to avoid noise,
 *   - emit only sales-vocabulary terms that the library actually indexes,
 *   - be reviewed when LEXICON_VERSION bumps.
 */
export const SALES_LEXICON: ReadonlyArray<LexiconEntry> = Object.freeze([
  // ── Change / migration / consolidation ────────────────────────────
  ...
  // ── Stage anchors (echo as discovery vocabulary) ──────────────────
...
/**
 * Token-bounded substring matcher. Triggers must be surrounded by
 * non-alphanumeric characters (or string boundaries). Lowercased input.
 */
...
export interface LexiconMatch {
  expansion: string;          // emitted sales term
  rule: string;               // entry.rule
  fromInput: string;          // the original raw seed that triggered it
}

/**
 * Scan a single raw seed against the lexicon. Returns all matched
 * expansions (deduped within this call by case-insensitive expansion).
 * Iteration order is stable: lexicon order, then expansion order.
 */
