...
class RepositoryArtifactTest(unittest.TestCase):
    ...
    def test_edge_function_inventory_is_reproducible(self):
        result = subprocess.run(
            [sys.executable, str(EDGE_TOOL), "--repo-root", str(ROOT)],
            check=False,
            capture_output=True,
            text=True,
        )
        ...
        self.assertEqual(generated, json.loads(EDGE_INVENTORY.read_text()))
        components = generated["components"]
        self.assertEqual(len(components), 120)
        self.assertEqual(sum(item["configuration_known"] for item in components), 42)
        self.assertEqual(
            sum("verify_jwt=true" in item["note"] for item in components), 5
        )
        self.assertEqual(
            sum("verify_jwt=false" in item["note"] for item in components), 37
        )
