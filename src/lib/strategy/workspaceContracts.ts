// ════════════════════════════════════════════════════════════════
// Workspace SOP — Contract Registry (client-side mirror of W1).
//
// The canonical registry lives in
// `supabase/functions/_shared/strategy-core/workspaceContracts.ts`
// (Deno runtime). This file is a structurally-identical mirror used
// by client code that cannot reach into the edge `_shared` tree.
//
// Both files share the same CONTRACT_VERSION; tests assert version
// equality so accidental drift is caught at CI time.
//
...
/**
 * Canonical workspace keys recognized by the W1 registry. Anything
 * outside this set is considered unknown and falls back to `work`
 * inside `normalizeWorkspaceKey`.
 */
const CANONICAL_KEY_SET = new Set<string>(ALL_WORKSPACE_KEYS);

/**
 * Aliases for legacy / camelCase / shorthand forms that may exist in
 * routes, settings, or older message payloads. Map → canonical key.
 *
 * Keep this list narrow. New canonical names go in WorkspaceKey, not
 * here. Custom workspaces (`custom:*`) are intentionally NOT aliased
 * — they fall through to the safe-fallback path.
 */
...
export interface NormalizeWorkspaceKeyResult {
  /** The resolved canonical key (always a valid WorkspaceKey). */
  key: WorkspaceKey;
  /** Where the resolution came from. */
  source: "canonical" | "alias" | "fallback";
  /** Original input value (string-coerced). */
  input: string | null;
  /** Optional human-readable note used by the resolver for telemetry. */
  note?: string;
}

/**
 * Normalize an arbitrary workspace identifier into a valid
 * `WorkspaceKey`. Unknown / null / custom workspace ids fall back to
 * `work` so downstream code never has to handle a missing contract.
 *
 * Callers that care about telemetry should inspect `.source` — a
 * `fallback` result indicates the input was unrecognized.
 */
export function normalizeWorkspaceKey(
  raw: unknown,
): NormalizeWorkspaceKeyResult {
  if (raw === null || raw === undefined) {
    return { key: "work", source: "fallback", input: null, note: "missing" };
  }
  ...
  if (!trimmed) {
    return { key: "work", source: "fallback", input: trimmed, note: "empty" };
  }
  if (CANONICAL_KEY_SET.has(trimmed)) {
    return {
      key: trimmed as WorkspaceKey,
      source: "canonical",
      input: trimmed,
    };
  }
  const alias = WORKSPACE_KEY_ALIASES[trimmed];
  if (alias) {
    return { key: alias, source: "alias", input: trimmed, note: `alias→${alias}` };
  }
  // Custom workspaces and anything else fall back to `work`.
  return {
    key: "work",
    source: "fallback",
    input: trimmed,
    note: trimmed.startsWith("custom:") ? "custom-workspace" : "unknown",
  };
}
