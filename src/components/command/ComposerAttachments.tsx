...
export interface Attachment {
  ...
  type: 'file' | 'image' | 'url';
  name: string;
  /** For files/images: the File object. For URLs: the URL string stored in `url`. */
  file?: File;
  url?: string;
...
export function ComposerAttachments({ attachments, onAdd, onRemove, disabled }: Props) {
    ...
    for (const file of fileArray) {
      ...
      if (isImage) {
        ...
        reader.onload = (e) => {
          ...
          // Force re-render by triggering onAdd with updated attachment
          onAdd([{ ...att }]);
        };
  ...
  return (
    ...
    >
      {/* Drop overlay */}
      {isDragging && (
        <div className="absolute inset-0 z-10 rounded-xl border-2 border-dashed border-primary/40 bg-primary/[0.04] flex items-center justify-center pointer-events-none">
      ...
      {(attachments.length > 0 || !disabled) && (
        <div className="flex items-center gap-1.5 flex-wrap mt-1.5">
            ...
            return (
              <span
                ...
                className={cn(
                  ...
                  'pl-1.5 pr-0.5 py-[2px] rounded-md border shrink-0',
                  'border-border/20 bg-muted/20 text-foreground/60',
                  'animate-in fade-in-0 zoom-in-95 duration-100',
                  att.type === 'image' && 'border-violet-500/15 bg-violet-500/[0.04]',
                  att.type === 'url' && 'border-sky-500/15 bg-sky-500/[0.04]',
                )}
              >
                {att.preview ? (
                  <img src={att.preview} alt="" className="h-3.5 w-3.5 rounded-sm object-cover" />
                ) : (
                  <Icon className={cn(
                    'h-2.5 w-2.5',
                    att.type === 'image' ? 'text-violet-400/60' : att.type === 'url' ? 'text-sky-400/60' : 'text-muted-foreground/50'
                  )} />
                )}
                <span className="truncate max-w-[120px]">{att.name}</span>
                {att.size && (
                  <span className="text-muted-foreground/30 text-[9px]">{formatSize(att.size)}</span>
                )}
                <button
                  ...
                  tabIndex={-1}
                >
                  <X className="h-2 w-2 text-muted-foreground/40 hover:text-muted-foreground/70" />
                </button>
              </span>
            );
          })}
          ...
          <button
            ...
            disabled={disabled}
            className={cn(
              'inline-flex items-center gap-0.5 text-[10px] text-muted-foreground/30',
              'hover:text-muted-foreground/50 px-1 py-0.5 rounded transition-colors duration-100',
              'hover:bg-muted/20',
