...
export function PowerHourModal({ open, onOpenChange }: PowerHourModalProps) {
  ...
  useEffect(() => {
    if (!open) {
      // Don't reset if session is running
      if (sessionState === 'setup' || sessionState === 'completed') {
  ...
  const applyToDaily = () => {
    ...
    for (let i = 0; i < dials; i++) {
      const hadConversation = i < connects;
  ...
  const decrementCounter = (setter: React.Dispatch<React.SetStateAction<number>>, current: number) => {
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between bg-secondary/50 rounded-lg p-3">
                ...
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    size="icon"
                    className="h-8 w-8"
                    onClick={() => decrementCounter(setDials, dials)}
                  >
                    <Minus className="h-4 w-4" />
                  </Button>
                  <span className="w-12 text-center text-2xl font-bold">{dials}</span>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-8 w-8"
                    onClick={() => incrementCounter(setDials)}
                  >
              ...
              <div className="flex items-center justify-between bg-secondary/50 rounded-lg p-3">
                ...
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    size="icon"
                    className="h-8 w-8"
                    onClick={() => decrementCounter(setConnects, connects)}
                  >
                    <Minus className="h-4 w-4" />
                  </Button>
                  <span className="w-12 text-center text-2xl font-bold">{connects}</span>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-8 w-8"
                    onClick={() => incrementCounter(setConnects)}
                  >
                    <Plus className="h-4 w-4" />
                  </Button>
                </div>
              ...
              <div className="flex items-center justify-between bg-secondary/50 rounded-lg p-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium">Meetings Set</span>
                </div>
                <div className="flex items-center gap-2">
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-8 w-8"
                    onClick={() => decrementCounter(setMeetingsSet, meetingsSet)}
                  >
                    <Minus className="h-4 w-4" />
                  </Button>
                  <span className="w-12 text-center text-2xl font-bold">{meetingsSet}</span>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-8 w-8"
                    onClick={() => incrementCounter(setMeetingsSet)}
                  >
                    <Plus className="h-4 w-4" />
                  </Button>
                </div>
            ...
            {sessionState === 'completed' && (
              <div className="text-center p-4 bg-status-green/10 rounded-lg">
                ...
                <p className="text-sm text-muted-foreground">
                  {dials} dials, {connects} connects, {meetingsSet} meetings
                </p>
