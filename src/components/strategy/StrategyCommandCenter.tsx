 ...
 *
 * CONTRAST HIERARCHY (enforced system-wide):
 *   Primary:     text-foreground          (titles, headings)
 *   Secondary:   text-foreground/80       (interactive labels, metadata you must read)
 *   Tertiary:    text-muted-foreground    (helper text, timestamps — still legible)
 *   Disabled:    text-muted-foreground/50 (truly disabled controls only)
 */
...
export function StrategyCommandCenter({ sidebarCollapsed, onExpandSidebar }: Props) {
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-background">
      {/* Toolbar */}
      <div className="flex items-center gap-2 px-4 h-9 border-b border-border/30 shrink-0">
        {sidebarCollapsed && (
          <Button size="sm" variant="ghost" className="h-6 px-1.5 text-[11px] text-muted-foreground" onClick={onExpandSidebar}>
            <PanelLeftOpen className="h-3 w-3 mr-1" /> Threads
          </Button>
        )}
        <span className="text-[10px] font-medium text-muted-foreground tracking-wider uppercase">Strategy</span>
      </div>

      <ScrollArea className="flex-1">
        <div className={cn(
          'flex flex-col items-center',
          isMobile ? 'px-4 pt-10 pb-8' : 'px-6 pt-14 pb-12',
          showEmpty && 'justify-center'
        )}>
          {/* Header — only on empty */}
          {showEmpty && (
            <div className="text-center mb-8 animate-in fade-in-0 duration-200">
              <h1 className="text-lg font-semibold text-foreground tracking-tight">What do you need?</h1>
              {kiCount > 0 && (
          ...
          <div className={cn(
            ...
            (result || isGenerating) && 'mb-8'
          )}>
            {/* Composer container — elevated surface */}
            <div className={cn(
              showEmpty && 'rounded-xl border border-border/30 bg-card/50 p-4 sm:p-5 shadow-sm'
            )}>
              <CommandBar
                accounts={accounts}
          ...
          {showEmpty && (
            <div className={cn(
              'w-full mt-8 animate-in fade-in-0 duration-150',
              isMobile ? 'max-w-full' : 'max-w-3xl'
            )}>
              {/* Pinned — go-to plays */}
              {pinnedShortcuts.length > 0 && (
                <div className="mb-6">
                  <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-[0.1em] mb-2.5 px-0.5 flex items-center gap-1">
                    ...
                    {pinnedShortcuts.map((s: any) => (
                      <button
                        ...
                        className={cn(
                          ...
                          'px-3 py-2 rounded-lg border border-primary/20 bg-primary/[0.06]',
                          'hover:bg-primary/[0.12] hover:border-primary/30',
              ...
              {/* Quick start */}
              <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-[0.1em] mb-2.5 px-0.5">
                Quick start
              </p>
              <div className={cn('grid gap-2', isMobile ? 'grid-cols-1' : 'grid-cols-2')}>
                {STARTERS.map(s => (
                  <button
                    ...
                    className={cn(
                      'group flex items-center gap-3 px-4 py-3 rounded-lg text-left',
                  ...
                  >
                    ...
                    <span className="text-[13px] text-foreground/75 group-hover:text-foreground transition-colors duration-100">{s.label}</span>
                  </button>
                ))}
              </div>
              ...
              {savedNonPinned.length > 0 && (
                <div className="mt-6">
                  <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-[0.1em] mb-2 px-0.5 flex items-center gap-1">
                    ...
                    {savedNonPinned.map((s: any) => (
                      <button
                        ...
                        className={cn(
                          ...
                          'px-2.5 py-1.5 rounded-md hover:bg-muted/40',
                          'transition-all duration-100 truncate max-w-[200px]'
                        )}
                      >
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {recents.map((r, i) => (
                      <button
                        ...
                        className="inline-flex items-center text-[11px] text-foreground/65 hover:text-foreground px-2.5 py-1.5 rounded-md hover:bg-muted/35 transition-all duration-100 truncate max-w-[200px]"
                      >
                        {r.label}
                      </button>
                    ))}
          ...
          {/* Output area */}
          {(result || isGenerating) && (
            <div className={cn('w-full', isMobile ? 'max-w-full' : 'max-w-3xl')}>
              {result && lastCommand && (
                <div className="flex items-center justify-end mb-2">
                  <button
                    onClick={handleSaveShortcut}
                    className="inline-flex items-center gap-1 text-[11px] text-muted-foreground hover:text-foreground transition-colors duration-100"
                  >
