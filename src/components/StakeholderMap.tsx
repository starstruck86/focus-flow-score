// Visual Buyer / Org Stakeholder Map for Accounts & Opportunities
...
import { Users, Sparkles, Plus, ExternalLink, Crown, Shield, Target, UserCheck, Lightbulb, Ban, ChevronRight, Linkedin } from 'lucide-react';
import { cn } from '@/lib/utils';

const BUYER_ROLES = [
  { value: 'economic_buyer', label: 'Economic Buyer', icon: Crown, color: 'text-amber-500', bg: 'bg-amber-500/10 border-amber-500/30' },
  { value: 'champion', label: 'Champion', icon: Shield, color: 'text-primary', bg: 'bg-primary/10 border-primary/30' },
  { value: 'technical_buyer', label: 'Technical Buyer', icon: Target, color: 'text-blue-500', bg: 'bg-blue-500/10 border-blue-500/30' },
  { value: 'user_buyer', label: 'User Buyer', icon: UserCheck, color: 'text-emerald-500', bg: 'bg-emerald-500/10 border-emerald-500/30' },
  { value: 'coach', label: 'Coach', icon: Lightbulb, color: 'text-purple-500', bg: 'bg-purple-500/10 border-purple-500/30' },
  { value: 'influencer', label: 'Influencer', icon: Users, color: 'text-cyan-500', bg: 'bg-cyan-500/10 border-cyan-500/30' },
  { value: 'blocker', label: 'Blocker', icon: Ban, color: 'text-destructive', bg: 'bg-destructive/10 border-destructive/30' },
  { value: 'unknown', label: 'Unknown', icon: Users, color: 'text-muted-foreground', bg: 'bg-muted/50 border-muted-foreground/20' },
] as const;
...
export function StakeholderMap({ accountId, accountName, website, industry, opportunityContext }: StakeholderMapProps) {
  ...
  const discoverContacts = useCallback(async () => {
    ...
    try {
      ...
      const discovered = data.contacts || [];
      if (discovered.length === 0) {
        toast.info('No new contacts discovered');
        return;
      }

      // Return discovered contacts for user to confirm
      return discovered;
    } catch (err) {
  ...
  const confirmContact = (contact: any) => {
    addContact.mutate({
      ...
      ai_discovered: true,
      discovery_source: contact.confidence || 'ai',
    });
            ...
            {discoveredContacts.map((c, i) => (
              <div key={i} className="flex items-center justify-between gap-2 p-2 rounded bg-background/80">
                ...
                <div className="flex gap-1">
                  <Button size="sm" variant="default" className="h-6 text-[10px] px-2" onClick={() => confirmContact(c)}>
                    Add
                  </Button>
                  <Button size="sm" variant="ghost" className="h-6 text-[10px] px-2" onClick={() => setDiscoveredContacts(prev => prev.filter((_, idx) => idx !== i))}>
                    Skip
                  </Button>
                </div>
              </div>
                        ...
                        return (
                          <>
                            <div className="grid grid-cols-2 gap-2">
                              <Select
                                value={(c as any).buyer_role || 'unknown'}
                                onValueChange={(v) => updateContact.mutate({ id: c.id, updates: { buyer_role: v } })}
                              >
                              ...
                              <Select
                                value={(c as any).influence_level || 'medium'}
                                onValueChange={(v) => updateContact.mutate({ id: c.id, updates: { influence_level: v } })}
                              >
                                <SelectTrigger className="h-7 text-xs"><SelectValue /></SelectTrigger>
                                <SelectContent>
                                  {INFLUENCE_LEVELS.map(l => (
                                    <SelectItem key={l} value={l} className="text-xs capitalize">{l} influence</SelectItem>
                                  ))}
                                </SelectContent>
                              </Select>
                            </div>
                            <Input
                              ...
                              onBlur={(e) => {
                                if (e.target.value !== (c.linkedin_url || '')) {
                                  updateContact.mutate({ id: c.id, updates: { linkedin_url: e.target.value || null } });
                                }
                              }}
                            />
                          </>
