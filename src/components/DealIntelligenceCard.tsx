...
export function DealIntelligenceCard({ opportunityId }: DealIntelligenceCardProps) {
  const { opportunities, accounts, contacts, methodology } = useStore();
  ...
  const signals = useMemo<DealSignals | null>(() => {
    ...
    const account = opp.accountId ? accounts.find(a => a.id === opp.accountId) : null;
    const oppContacts = opp.accountId
      ? contacts.filter(c => c.accountId === opp.accountId)
      : [];
    const meth = methodology.find(m => m.opportunityId === opportunityId);
    ...
    } else if (daysToClose !== null && daysToClose < 14 && opp.stage !== 'Negotiate' && opp.stage !== 'Closed Won') {
      risks.push({ text: `${daysToClose}d to close but still in ${opp.stage || 'early'} stage`, severity: 75 });
    }
    ...
    if (meth) {
      const meddiccFields = [
        { key: 'metricsConfirmed', label: 'Metrics' },
        { key: 'economicBuyerConfirmed', label: 'Economic Buyer' },
        { key: 'decisionCriteriaConfirmed', label: 'Decision Criteria' },
        { key: 'decisionProcessConfirmed', label: 'Decision Process' },
        { key: 'identifyPainConfirmed', label: 'Pain' },
        { key: 'championConfirmed', label: 'Champion' },
        { key: 'competitionConfirmed', label: 'Competition' },
      ] as const;
    ...
    } else if (opp.stage && opp.stage !== '' && opp.stage !== 'Prospect') {
  ...
  }, [opp, accounts, contacts, methodology, opportunityId, playbookRec]);
