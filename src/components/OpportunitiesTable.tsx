...
export function OpportunitiesTable({ onOpenDrawer, renewalsOnly = false, excludeRenewals = false, showChurnRisk = true, columnOrder = 'default', stageFilter, onClearStageFilter, highlightId }: OpportunitiesTableProps) {
  ...
  const renderOpportunityRow = (opp: Opportunity) => {
    ...
    if (columnOrder === 'outreach') {
      ...
      return (
        <React.Fragment key={opp.id}>
          <TableRow data-testid={`opp-row-${opp.id}`} data-opp-id={opp.id} className={cn("group hover:bg-muted/30 cursor-pointer", localHighlight === opp.id && "ring-2 ring-primary/50 bg-primary/5 animate-pulse")} onClick={() => toggleExpand(opp.id)}>
    ...
    if (renewalsOnly) {
      ...
      return (
        <React.Fragment key={opp.id}>
           <TableRow data-testid={`renewal-row-${opp.id}`} data-opp-id={opp.id} className={cn("group hover:bg-muted/30 cursor-pointer", localHighlight === opp.id && "ring-2 ring-primary/50 bg-primary/5 animate-pulse")} onClick={() => toggleExpand(opp.id)}>
            <TableCell className="w-8 py-3" onClick={(e) => e.stopPropagation()}>
              <Checkbox checked={bulkSelection.isSelected(opp.id)} onCheckedChange={() => bulkSelection.toggle(opp.id)} />
            </TableCell>
            <TableCell className="w-8 py-3">
