 ...
 * confirmation modal, and coverage lift summary.
 */
...
import {
  Zap, Loader2, CheckCircle2, XCircle, AlertTriangle, TrendingUp, Trash2,
} from 'lucide-react';
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-[10px] text-right">Δ KIs</TableHead>
              ...
              <TableBody>
                {queue.map(item => (
                  <TableRow key={item.resource_id}>
                    ...
                    <TableCell className="text-[11px] max-w-[140px] truncate">{item.title}</TableCell>
                    <TableCell className="text-[11px] text-right font-mono">{(item.content_length / 1000).toFixed(1)}k</TableCell>
                    <TableCell className="text-[11px] text-right font-mono">
                      {item.pre_ki_count} <span className="text-muted-foreground">({item.pre_kis_per_1k}/1k)</span>
                    </TableCell>
                    <TableCell className="text-[11px] text-right font-mono">
                      {item.post_ki_count != null ? (
                        <>{item.post_ki_count} <span className="text-muted-foreground">({item.post_kis_per_1k}/1k)</span></>
                      ) : '—'}
                    </TableCell>
                    <TableCell className={cn(
                      "text-[11px] text-right font-mono font-bold",
                      (item.ki_delta ?? 0) > 0 ? "text-emerald-600" : (item.ki_delta ?? 0) < 0 ? "text-destructive" : ""
                    )}>
                      {item.ki_delta != null ? (item.ki_delta > 0 ? `+${item.ki_delta}` : item.ki_delta) : '—'}
                    </TableCell>
                    <TableCell className="text-[10px] text-muted-foreground max-w-[100px] truncate">{item.reason}</TableCell>
                    <TableCell>
          ...
          <CardContent>
            <div className="grid grid-cols-3 gap-2 text-center text-xs">
              ...
              <div className="border border-border rounded-md p-2">
                ...
                <div className="text-muted-foreground">KI Delta</div>
              </div>
              <div className="border border-border rounded-md p-2">
