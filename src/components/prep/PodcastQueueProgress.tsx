...
import { CheckCircle2, XCircle, Loader2, Clock, Ban, Trash2, Brain, AlertTriangle, ChevronDown, ChevronRight, Sparkles, FileText, ShieldCheck, Eye, ThumbsDown, RefreshCw, Filter, RotateCcw } from 'lucide-react';
import type { QueueItem, QueueStats } from '@/hooks/usePodcastQueue';
import { canApproveItem } from '@/hooks/usePodcastQueue';
...
interface PodcastQueueProgressProps {
  ...
  onReprocessStructure?: (queueItemId: string) => void;
  onReprocessFull?: (queueItemId: string) => void;
...
export function PodcastQueueProgress({
  ...
  onRejectTranscript, onReprocessStructure, onReprocessFull, generatingKIs,
        ...
        <ScrollArea className="max-h-[280px] border rounded-md">
          <div className="p-2 space-y-0.5">
            {filteredItems.map(item => {
              ...
              const isAwaiting = item.ki_status === 'awaiting_approval' && item.resource_id;
              const isRejected = item.ki_status === 'rejected';
              const canGenKI = item.ki_status === 'ready_for_review' && item.resource_id && !isGenerating;
              const approvalCheck = isAwaiting ? canApproveItem(item) : null;
              const hasRawTranscript = !!item.raw_transcript;
                ...
                <Collapsible key={item.id} open={isExpanded} onOpenChange={() => setExpandedItem(isExpanded ? null : item.id)}>
                  <CollapsibleTrigger asChild>
                    <div className="flex items-center gap-2 py-1.5 px-2 rounded text-xs hover:bg-muted/50 cursor-pointer">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        {transcript && <span className={`text-[10px] ${transcript.color}`}>{transcript.text}</span>}
                        {ki && <span className={`text-[10px] ${ki.color}`}>{ki.text}</span>}
                        {failure && (
                  ...
                  <CollapsibleContent>
                    <div className="ml-5 pl-2 border-l border-border space-y-2 py-2 text-[11px]">
                      ...
                      {/* Review reason (separate from error) */}
                      {item.review_reason && (
                        <div className="text-[10px] text-muted-foreground italic">
                          Rejection reason: {item.review_reason}
                        </div>
                      )}
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    className="text-[10px] h-6 gap-1 opacity-50 cursor-not-allowed"
                                    disabled
                                  >
                                    <ShieldCheck className="h-3 w-3" /> Approve
                                  </Button>
                      ...
                      {/* Generate KIs — only after approval, never for rejected */}
                      {canGenKI && onGenerateKIs && (
                        <Button
                          variant="outline"
                          size="sm"
                          className="text-[10px] h-6 gap-1"
                      ...
                      {/* Reprocess actions for rejected / failed items */}
                      {(isRejected || item.status === 'failed') && (onReprocessStructure || onReprocessFull) && (
                        <div className="flex items-center gap-1.5">
                          {hasRawTranscript && onReprocessStructure && (
                            <Button
                              variant="outline"
                              size="sm"
                              className="text-[10px] h-6 gap-1"
                              onClick={(e) => { e.stopPropagation(); onReprocessStructure(item.id); }}
                            >
                              <RefreshCw className="h-3 w-3" /> Reprocess Structure
                            </Button>
                          )}
                          {onReprocessFull && (
                            <Button
                              ...
                              size="sm"
                              className="text-[10px] h-6 gap-1"
                              onClick={(e) => { e.stopPropagation(); onReprocessFull(item.id); }}
                            >
                              <RotateCcw className="h-3 w-3" /> Reprocess Full
                            </Button>
                          )}
                        </div>
                      )}
