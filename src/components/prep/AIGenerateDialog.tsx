...
export function AIGenerateDialog({ open, onOpenChange, onGenerated, accountContext }: AIGenerateDialogProps) {
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Sparkles className="h-4 w-4 text-primary" /> AI Document Generator
          </DialogTitle>
        </DialogHeader>

        <div className="space-y-4">
          <div>
            <label className="text-xs font-medium text-muted-foreground mb-1 block">What do you want to create?</label>
            <Textarea
              value={prompt}
              onChange={e => setPrompt(e.target.value)}
              placeholder="e.g., Write a discovery prep brief for our upcoming call with the VP of Marketing..."
