...
export function ResourceManager() {
  ...
  const handleCreateResource = useCallback(() => {
    ...
    createResource.mutate({
      ...
      onSuccess: (data) => {
        setEditingResource(data as Resource);
      },
    });
    ...
    return (
      <VersionHistory
        ...
        onRestoreVersion={(content) => {
          // Will be handled by the version history component
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <Button size="sm" className="h-8 text-xs" onClick={() => setShowNewResource(true)}>
          <FilePlus className="h-3.5 w-3.5 mr-1" /> New Resource
        </Button>
      </div>
      ...
      {filteredResources.length > 0 ? (
        ...
        <div className="text-center py-12 text-muted-foreground">
          ...
          <p className="text-xs mt-1">Create your first document, template, or prep brief</p>
        </div>
      ) : null}
