...
export async function generateContent(ctx: ToolContext, params: { contentType: string; accountName?: string; opportunityName?: string; contactName?: string; customInstructions?: string }): Promise<string> {
  ...
  if (params.accountName) {
    const { data: accounts } = await supabase.from('accounts').select('id, name, industry, notes').eq('user_id', userId).is('deleted_at', null).ilike('name', `%${params.accountName}%`).limit(1);
    if (accounts?.length) {
...
export async function meetingBrief(ctx: ToolContext, params: { meetingTitle?: string }): Promise<string> {
  ...
  const { data: accounts } = await supabase.from('accounts').select('id, name, industry, tier, notes, last_touch_date, account_status').eq('user_id', userId).is('deleted_at', null);
