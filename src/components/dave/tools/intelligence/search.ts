...
export async function searchCrm(ctx: ToolContext, params: { query: string }): Promise<string> {
  ...
  const [accts, opps, contacts, transcripts] = await Promise.all([
    supabase.from('accounts').select('name, tier, account_status').eq('user_id', userId).is('deleted_at', null).or(`name.ilike.${q},notes.ilike.${q},industry.ilike.${q}`).limit(5),
...
export async function competitiveIntel(ctx: ToolContext, params: { query: string }): Promise<string> {
  ...
  const { data: accounts } = await supabase.from('accounts').select('id, name, notes').eq('user_id', userId).is('deleted_at', null).ilike('notes', `%${params.query}%`).limit(10);
