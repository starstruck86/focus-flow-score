...
export default function BatchRegrade() {
  ...
  const run = async () => {
    ...
    // Load already-completed IDs from DB (regraded_at IS NOT NULL)
    ...
    const doneIds = new Set((done || []).map((r: any) => r.transcript_id));
    addLog(`${doneIds.size} already done in DB, ${TRANSCRIPT_IDS.length - doneIds.size} to process`);

    for (let i = 0; i < TRANSCRIPT_IDS.length; i++) {
      const id = TRANSCRIPT_IDS[i];

      if (doneIds.has(id)) {
        setResults(prev => prev.map(r => r.id === id ? { ...r, status: 'skipped' } : r));
        continue;
      }

      setResults(prev => prev.map(r => r.id === id ? { ...r, status: 'running' } : r));
      addLog(`[${i + 1}/${TRANSCRIPT_IDS.length}] ${id.slice(0, 8)}…`);

      try {
        const result = await gradeOne(id, session.access_token);
        if (result) {
          // Stamp regraded_at in DB so this call is skipped on future runs
      ...
      if (i < TRANSCRIPT_IDS.length - 1) await sleep(4000);
    }
    ...
    <div className="min-h-screen bg-background px-4 py-8 space-y-5">
      <div className="max-w-md mx-auto space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {isComplete
            ? `Done — ${done} re-graded, ${skipped} skipped, ${errors} failed`
      ...
      <div className="max-w-md mx-auto space-y-1">
        {results.map(r => (
          <div key={r.id} className={cn(
            ...
            r.status === 'skipped' ? 'bg-muted/10 border border-border/20' :
            ...
            <div className="flex items-center gap-2">
              ...
              {r.status === 'skipped' && <SkipForward className="h-3.5 w-3.5 text-muted-foreground/40" />}
              ...
              {r.status === 'pending' && <span className="text-xs text-muted-foreground/20">—</span>}
            </div>
          </div>
        ))}
      </div>
      ...
      {log.length > 0 && (
        <div className="max-w-md mx-auto bg-muted/20 rounded-lg p-3 max-h-32 overflow-y-auto">
