...
function DaveHealthSection() {
  ...
  const runCheck = async () => {
    setLoading(true);
    try {
      const { data } = await trackedInvoke('dave-health-check');
      if (data) {
        setHealth({
  ...
  const runSmokeTest = async () => {
    ...
    try {
      ...
      try {
        const { data: hcData } = await trackedInvoke('dave-health-check');
        if (hcData) {
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" onClick={async () => {
            setRegistering(true);
            try {
              const { data, error } = await trackedInvoke('register-dave-tools');
               ...
               <div className="metric-card">
                 <div className="flex items-center justify-between">
                   <div className="flex items-center gap-3">
                     ...
                     <div>
                       <h3 className="font-semibold">"Hey Dave" Wake Word</h3>
