...
- **SUPABASE MIGRATION REHEARSAL — INSPECTION DIAGNOSTICS HARDENED IN DRAFT; READINESS RED.** PR #13 and ZIP-envelope PR #14 were independently merged with history-preserving merge commits; verified main before this work is `4fac3414f46f56f961574cea8ec99f1c439e7112`. One authorized rehearsal export remains retained offline in the approved evidence store. A separately authorized metadata-only attempt failed closed inside the raw inspector without stable stage attribution and produced no normal completed evidence package; no identifying artifact path, name, size, hash, or timestamp is recorded here. Draft PR #15 at substantive head `65281d0597c8339882f2e70d8f0119dc5f1948d5` makes every raw-inspector failure a canonical allowlisted stage/reason record, suppresses all raw child output on failure, captures safe PGDMP header bytes before `pg_restore` and binds them to the verified inner SHA-256, records and isolates the exact execution Python, removes inherited shell/Python startup hooks, and completes private cleanup before atomic no-replace report publication. Planted failures cover every stage, malformed diagnostics, startup substitution, reviewed PostgreSQL 17 reason classes, report staging/publication, and secret/row-payload nonleakage. Local synthetic validation is green (116 migration Python tests, 74 raw-inspector checks, and 26 target-safety regressions); Linux PostgreSQL 17 and macOS remain required final-head CI gates. The tooling verifies local byte handling, supplied identity equality, checked-out tool/runtime identity, provenance structure, and what `pg_restore --list` reports. It still cannot prove Lovable source completeness or UI-to-backend mapping, independently attest operator/UI claims, prove every inner byte was consumed, validate a restore, or establish target readiness. PR #15 did not access or retry inspection of the retained export and did not access Lovable, Supabase, production, secrets, or any external/pre-existing database; it performed no restore, deployment, migration, export, or data movement. `.lovable/plan.md` remains rejected historical input.
