...
**Updated:** July 15, 2026 (v21 — repository containment status refreshed;
production/runtime baseline unchanged)
...
- **DYNAMIC-STAGING CREDENTIAL CONTAINMENT — PR #17 DRAFT; EXTERNAL ROTATION BLOCKER OPEN.** Draft PR #17 at latest substantive head `00680fdf0d766912bfc4be86ffa8791925e8f7d3` removes credential-shaped literals and the executable schedule block from the current derived `supabase/dynamic_staging_schema.sql` snapshot. That snapshot is non-executable repository drift evidence outside chronological migrations; no runtime schedule was queried, disabled, removed, recreated, or changed, and runtime state remains unverified. Historical repository evidence recorded nine snapshot schedules total: migration-defined `ops_sentinel_v1` plus eight additional schedules. The sanitized snapshot has no reviewed executable-cron, staging/production project-binding, cron-header, or bearer-JWT patterns. The new scanner is a known-pattern accidental-regression guard over every tracked `.sql` file and only tracked `.ini`, `.json`, `.toml`, `.yaml`, and `.yml` files under `supabase/`; it is not an exhaustive SQL/configuration security boundary. Except for the one reviewed derived-snapshot path, diagnostics replace filenames with an opaque artifact label. Prior independent repository verification—not this category-only scanner—classified the removed bearer material as anon/publishable client material rather than service-role authority; it still must not be hardcoded into cron SQL. The public PR patch disclosed predecessor bytes despite the removed diff-rendering hint, so the PR patch, Git history, forks, caches, archived patches, and prior clones are persistent exposure surfaces. External rotation of the staging cron secret is the required open mitigation; this PR does not rotate or verify it, and history rewriting remains out of scope. Recreating jobs initially disabled is operator policy, not a repository-enforced state. No Lovable, Supabase, n8n, staging, production, secret store, external or pre-existing database, or retained export was accessed or changed; CI validation uses only isolated synthetic PostgreSQL fixtures.
