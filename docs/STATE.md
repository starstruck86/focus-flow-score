...
- **DYNAMIC-STAGING CREDENTIAL CONTAINMENT — DRAFT; ROTATION PENDING.** Draft PR #17 at latest substantive head `8068b7248a73f962e1bf5ee001fb2e300f5ddec6` removes the current-tree credential-shaped literals and the complete executable schedule block from `supabase/dynamic_staging_schema.sql`, which is derived drift evidence outside chronological migrations and is not an executable restore source. Scheduling is fail closed: any future recreation requires separate deployment authorization, reviewed endpoints/configuration, an initially disabled state, and an approved runtime secret source with no functional default. A tracked-artifact scanner now covers SQL and Supabase server configuration, emits only canonical path/finding-category records, rejects embedded bearer JWTs, non-placeholder cron-header values, unsafe runtime-secret assignments/defaults, and executable cron reintroduction in the derived snapshot, and fails closed on ambiguous scan inputs. The removed bearer material was anon/publishable client material, not service-role authority, but it must not be embedded in server-side cron SQL. The staging cron shared secret still requires external rotation; this draft neither rotates nor verifies it. Current-tree removal does not remove prior exposure from Git history, forks, caches, archived patches, or prior clones, and history rewriting is explicitly out of scope. No Lovable, Supabase, n8n, staging, production, secret store, database, or retained export was accessed or changed.
