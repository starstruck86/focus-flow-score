...
- **SUPABASE MIGRATION REHEARSAL — ZIP INSPECTION TOOLING IN DRAFT; READINESS RED.** PR #13 was independently merged with a history-preserving merge at verified main `ecdef62656c793b62611d89be22b09d15a03e74d`; it established the Support-reported export/remix plan, repository inventory, raw-`PGDMP` inspector, verification tools, and fail-closed rehearsal/cutover gates. One authorized rehearsal export is retained offline in the approved evidence store, but metadata inspection stopped because its canonical outer artifact is ZIP-wrapped while the reviewed procedure accepted direct `PGDMP`. No identifying artifact path, name, size, hash, or timestamp is recorded here. Draft PR #14 adds synthetic-only ZIP-envelope support at substantive head `19c8e4187311847bf5c48ffe4a3804b7934cc850`: one strictly framed regular member, bounded streaming extraction to a fixed private file, separate outer/member/inner/report/provenance identities, atomic no-replace publication, unchanged direct-inspector restrictions, and structured incomplete-timeline evidence. The retained rehearsal's missing initiation observation is irreparable and must remain explicit; it does not require another export or prevent a separately authorized offline inspection after review. Future and final exports must record initiation correctly. The tooling verifies local byte handling, provenance structure, and what `pg_restore --list` reports; it cannot prove Lovable source completeness, independently attest operator/UI claims, validate a restore, or establish target readiness. PR #14 did not access the retained export, Lovable, Supabase, production, secrets, or any remote/pre-existing database, and performed no restore, deployment, migration, export, or data movement. `.lovable/plan.md` remains rejected historical input.
