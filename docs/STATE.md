...
- **SUPABASE MIGRATION REHEARSAL — ZIP INSPECTION TOOLING HARDENED IN DRAFT; READINESS RED.** PR #13 was independently merged with a history-preserving merge at verified main `ecdef62656c793b62611d89be22b09d15a03e74d`; it established the Support-reported export/remix plan, repository inventory, raw-`PGDMP` inspector, verification tools, and fail-closed rehearsal/cutover gates. One authorized rehearsal export is retained offline in the approved evidence store, but metadata inspection remains stopped because its ZIP envelope requires a merged, externally approved execution procedure. No identifying artifact path, name, size, hash, or timestamp is recorded here. Draft PR #14 is hardened at substantive head `e4120f0d731a17850890d5bbc20ad8b26144c646`: mandatory externally supplied outer filename/size/SHA and owner-only store-root binding; exact approved-checkout `supabase/config.toml` project-ref and ZIP-member/UI-name equality; strict one-member streaming normalization; fixed resource bounds around `pg_restore --version`/`--list`; descriptor-relative, no-replace durable publication; and a persistent `EVIDENCE_INDETERMINATE` state that cannot pass the completion validator after a postcommit failure. Synthetic local validation is green (109 migration Python tests, 39 raw-inspector tests, and 26 target-safety regressions); the PR adds real PostgreSQL 17 high-level ZIP/direct integration on Linux, executed from a clean isolated clone of the exact checkout, and the actual macOS `renameatx_np` path for final CI. The retained rehearsal's missing initiation observation remains irreparable and explicit; future/final exports must record initiation correctly. The tooling verifies local byte handling, externally supplied identity equality, provenance structure, and what `pg_restore --list` reports. It cannot prove Lovable source completeness or UI-to-backend mapping, independently attest operator/UI claims, prove every direct-PGDMP byte was consumed, validate a restore, or establish target readiness; filesystem crash/I/O ambiguity at the final rename/`fsync` boundary is a manual-quarantine stop, never a green result. PR #14 did not access the retained export, Lovable, Supabase, production, secrets, or any external/pre-existing database; its PostgreSQL CI fixture is isolated and synthetic. It performed no restore, deployment, migration, export, or data movement. `.lovable/plan.md` remains rejected historical input.
...
- **SUPABASE MIGRATION to own account:** PR #13 is merged; ZIP-envelope inspection support is open in draft PR #14. One authorized rehearsal export is retained offline but remains uninspected pending procedure review. No restore, target project creation/access, remix, or cutover has occurred. Production remains on Lovable-managed `odbjjklumdsuqdvkgwyv`. Keep the original project intact and any future target read-only until rollback-critical verification passes.
