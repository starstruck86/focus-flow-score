...
- **ALL-FUNCTION RUNTIME ATTESTATION — IN REVIEW, NOT DEPLOYED.** Draft PR #12 (`agent/all-functions-runtime-attestation`, substantive head `dd649ec63017058c77b896aa766897b5aa41a8f4`) replaces the hand-editable version-only SHA authority with an ignored manifest generated from GitHub Actions `GITHUB_SHA`. Each of `strategy-chat`, `analyze-call`, `mcp`, and `version` reports its bundle SHA and deployment ID. A dispatch-only workflow validates one clean detached `main` checkout, stages all four functions in one CLI invocation, and fails unless every runtime reports the expected SHA. Before use, GitHub needs a protected `production` environment with `SUPABASE_ACCESS_TOKEN`; main and out-of-band Supabase deploy authority also need protection for CI to be the provenance boundary.
