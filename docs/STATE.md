...
- **SUPABASE MIGRATION REHEARSAL — DRAFT, NOT EXECUTED.** Draft PR #13 pivots the migration architecture from the rejected custom bridge design to Lovable Support's reported Cloud export/remix path. Its hardening adds provenance-bound/fail-closed manifests, resolved Edge Function deployment-closure fingerprints and structured effective `verify_jwt`, a deterministic PostgreSQL catalog-output converter without PK values/ranges, immutable dump/TOC binding, real PostgreSQL 17 CI integration, and enforced source/target write-gate and lifecycle gates in the runbook. A follow-up closed a CI false-green caused by unavailable `rg` commands being swallowed inside shell conditionals: assertions now preflight fixed `grep`, distinguish no-match from command failure, avoid optimization-removable Python assertions, and run only after strict local test-target admission. The PostgreSQL harnesses require explicit fixture opt-in, a `migration_verify_*` database, PostgreSQL 17 identity, and either a canonical local Unix socket or a labeled local test container; 26 planted failures prove unsafe targets and missing/erroring dependencies do not reach fixture SQL. Metadata-only inspection now requires a canonical archive in an approved encrypted evidence store, a hash-verified disposable working copy, exact external-before/external-after/report archive SHA equality, and an externally supplied `APPROVED_EXECUTION_CHECKOUT_SHA` with no default that must identify and exactly equal the executing HEAD before any run directory or `pg_restore` call. Provenance separately records that approval pin, the runtime-resolved execution checkout, the committed README blob, the exact marked fenced-block SHA-256, informational historical origin `e4eed4a21049d274738110710a468e265c2893d2`, and unchanged inspector/helper/migration baseline `c87a124602eb669b3ec5a3829610c6cb465d3e26`. Tracked tool/baseline changes plus ordinary or ignored untracked migration files fail before inspection. Synthetic end-to-end coverage executes the checked-in workflow through the real inspector/helper with a controlled fake `pg_restore`; a clean committed-descendant regression proves a previously approved ancestor fails and the descendant proceeds only when its exact new HEAD is supplied. This verifies local procedure mechanics, supplied-pin equality, and checked-out content identity only; it does not prove who approved the pin, attest separately edited shell input, validate a real Lovable archive, prove source completeness, establish remote state, or validate restore compatibility. The latest substantive branch head is `df0a28d0409b6123e74179f41c625f5b20a168f7`. Support's export-format, restore, Auth, Storage, Remix, write-fence, and rate-limit statements remain unproven until an authorized empirical rehearsal or written confirmation. No project was created or queried; no export, restore, remix, deployment, secret access, production invocation, or data movement occurred. `.lovable/plan.md` is explicitly rejected historical input, and the preserved local bridge spike is not part of PR #13.
