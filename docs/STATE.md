...
- **SUPABASE MIGRATION REHEARSAL — TOC ANALYSIS CORRECTION IN DRAFT; READINESS RED.** PR #13, ZIP-envelope PR #14, and diagnostic PR #15 are merged with history-preserving merge commits; PR #15 is present in verified main `8b872882787859b87549e7f884832c624e29ead9`. One authorized rehearsal export remains retained offline in the approved evidence store. A separately authorized metadata-only attempt was reported to have failed closed at `report_helper_failed` / `unresolved_known_toc_entry`; it produced no evidence package and performed no restore. No identifying artifact path, name, size, hash, or timestamp is recorded here. Draft PR #16 at latest substantive head `fbf78cfacd3e125021d195637cfd4aaac1353c30` reproduces the starting-main false fatal with synthetic input, adds a narrow class-specific ASCII grammar for hyphenated EXTENSION names, and separates durable evidence completion from object-reference analysis. A recognized non-exempt TOC class that cannot be resolved now retains only aggregate metadata with `object_reference_analysis=INCOMPLETE`, `migration_duplicate_analysis=INCOMPLETE`, and `restore_planning_gate=BLOCKED`; raw TOC lines, names, schemas, owners, OIDs, SQL, paths, and payloads are excluded from that report path. The aggregate-only report is accepted only as an exact LF-terminated canonical sequence with one copy of each field, a fixed ordered class ledger, and internally consistent TOC arithmetic; fully rehashed duplicate or contradictory fields fail both pending and descriptor-bound durable validation. Unknown classes, malformed TOCs, duplicate IDs, conflicting version headers, archive/hash failures, and unsafe diagnostics remain fatal. Provenance format version 5, report/provenance cross-validation, stdout, and `EVIDENCE_COMPLETE` all bind the hard `BLOCKED` gate, so evidence-package completion cannot be interpreted as restore readiness or migration green. Local synthetic validation is green (140 migration Python tests, 111 raw-inspector checks, and 26 target-safety regressions); Linux PostgreSQL 17 plus application, Deno, audit, and macOS final-head CI remain required. The tooling still cannot prove Lovable source completeness or UI-to-backend mapping, independently attest operator/UI claims, prove every inner byte was consumed, validate a restore, or establish target readiness. PR #16 did not access, stat, hash, read, copy, rename, chmod, unzip, normalize, or inspect the retained export and did not access Lovable, Supabase, production, secrets, or any external/pre-existing database; it performed no restore, deployment, migration, export, or data movement. `.lovable/plan.md` remains rejected historical input.
