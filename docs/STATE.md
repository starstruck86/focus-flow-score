...
- **SUPABASE MIGRATION REHEARSAL — HELPER DIAGNOSTICS HARDENED IN DRAFT; READINESS RED.** PR #13 and ZIP-envelope PR #14 were independently merged with history-preserving merge commits; verified main before this work is `4fac3414f46f56f961574cea8ec99f1c439e7112`. One authorized rehearsal export remains retained offline in the approved evidence store. A separately authorized metadata-only attempt failed closed inside the raw inspector and produced no normal completed evidence package; no identifying artifact path, name, size, hash, or timestamp is recorded here. Draft PR #15 at substantive head `687ff5ee19127d0094d100d5326eaf98bf7eae26` now gives the report helper one exact ASCII JSON protocol with eight reviewed reasons, privately reduces every malformed or unapproved helper record to `other_nonzero`, and accepts only reviewed raw-inspector stage/reason combinations at the high-level boundary. Partial poisoned reports, derived bytes, working copies, manifests, completion markers, and durable runs are removed on failure. The helper and raw inspector are bound to the exact externally approved execution checkout by Git blob and file SHA-256; historical baseline `c87a124602eb669b3ec5a3829610c6cb465d3e26` now honestly covers only the unchanged migration-input tree. Standalone report publication proves rollback after post-link durability failure or leaves a fixed private `INDETERMINATE` marker/named quarantine; a committed report cannot be relabeled failed merely because its best-effort success notification cannot write. Direct stdout compatibility remains nontransactional and is not durable evidence; the evidence workflow always uses `--output`. Local synthetic validation is green (125 migration Python tests, 109 raw-inspector checks, and 26 target-safety regressions); Linux PostgreSQL 17 and macOS remain required final-head CI gates. The tooling verifies local byte handling, supplied identity equality, checked-out tool/runtime identity, provenance structure, and what `pg_restore --list` reports. It still cannot prove Lovable source completeness or UI-to-backend mapping, independently attest operator/UI claims, prove every inner byte was consumed, validate a restore, or establish target readiness. PR #15 did not access, stat, hash, read, copy, rename, chmod, unzip, or retry inspection of the retained export and did not access Lovable, Supabase, production, secrets, or any external/pre-existing database; it performed no restore, deployment, migration, export, or data movement. `.lovable/plan.md` remains rejected historical input.
...
- **SUPABASE MIGRATION to own account:** PR #13 and ZIP-envelope PR #14 are merged; diagnostic hardening is open in draft PR #15. One authorized rehearsal export is retained offline, but no completed metadata evidence package exists and no inspection retry is authorized by this PR. No restore, target project creation/access, remix, or cutover has occurred. Production remains on Lovable-managed `odbjjklumdsuqdvkgwyv`. Keep the original project intact and any future target read-only until rollback-critical verification passes.
