...
- Explicit true (6): analyze-sentiment, car-mode-score, dave-health-check, dojo-score, drill-review, run-strategy-task-reaper-receipt-v1.
...
- cronHeadReceiver.ts (through each function's local `handler.ts`): daily-digest, run-strategy-task-reaper, run-strategy-task-reaper-receipt-v1, schedule-daily-plan.
- cronSecretAuth.ts (through cronHeadReceiver.ts): daily-digest, run-strategy-task-reaper, run-strategy-task-reaper-receipt-v1, schedule-daily-plan.
- cronAttemptReceipt.ts (through the strict local handler only): run-strategy-task-reaper-receipt-v1. The legacy `run-strategy-task-reaper` closure deliberately excludes this module.
...
| `run-strategy-task-reaper-receipt-v1` | true | `cronAttemptReceipt.ts`<br>`cronHeadReceiver.ts`<br>`cronSecretAuth.ts` | `CRON_SECRET`<br>`CRON_SECRET_NEXT`<br>`SUPABASE_SERVICE_ROLE_KEY`<br>`SUPABASE_URL` |
